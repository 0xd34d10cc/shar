...
static int run() {
  auto logger = shar::Logger("sender.log");
  auto metrics = std::make_shared<shar::Metrics>(20, logger);
  auto context = shar::ProcessorContext{"", // name
                                        logger, metrics};
  ...
  auto monitor = sc::GetMonitors().front();
  std::size_t width =
      config.get<std::size_t>("width", static_cast<std::size_t>(monitor.Width));
  std::size_t height = config.get<std::size_t>(
  ...
  auto capture = std::make_shared<shar::ScreenCapture>(
      context.with_name("Capture"), interval, monitor,
      std::move(captured_frames_sender));
  auto encoder = std::make_shared<shar::H264Encoder>(
      context.with_name("Encoder"), frame_size, fps, encoder_config,
      std::move(captured_frames_receiver), std::move(encoded_packets_sender));
