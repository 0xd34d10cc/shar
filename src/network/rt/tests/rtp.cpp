...
TEST(rtp, empty_packet) {
    std::array<std::uint8_t, rtp::Packet::MIN_HEADER_SIZE> data{};
    rtp::Packet packet{data.data(), data.size()};
...
TEST(rtp, set_packet_fields) {
    ...
    EXPECT_EQ(packet.stream_id(), 12);
}

TEST(rtp, deserialize) {
    // copy-pasted from wireshark
    const std::size_t packet_size = 1123;
    const char* raw_data = "\x80\x60\x32\x3a\x03\x4a" \
...
"\x2d\xa4\xbf\xab\x14";
    ...
    std::vector<std::uint8_t> buffer{
        ...
        reinterpret_cast<const std::uint8_t*>(raw_data) + packet_size - 8 // udp header size
    };

    rtp::Packet packet{buffer.data(), buffer.size()};

    // all constants were copy-pasted from wireshark
    EXPECT_EQ(packet.version(), 2);
    EXPECT_EQ(packet.has_padding(), false);
    EXPECT_EQ(packet.has_extensions(), false);
    EXPECT_EQ(packet.contributors_count(), 0);
