#pragma once

#include <cstdint> // std::uint8_t
#include <cstdlib> // std::size_t


namespace shar::rtp {

//  The RTP header has the following format:
//   0               1               2               3
//   7 6 5 4 3 2 1 0 7 6 5 4 3 2 1 0 7 6 5 4 3 2 1 0 7 6 5 4 3 2 1 0
//  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
//  |V=2|P|X|  CC   |M|     PT      |       sequence number         |
//  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
//  |                           timestamp                           |
//  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
//  |           synchronization source (SSRC) identifier            |
//  +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
//  |            contributing source (CSRC) identifiers             |
//  |                             ....                              |
//  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
// The first twelve octets are present in every RTP packet, while the
// list of CSRC identifiers is present only when inserted by a mixer.
class Packet {
public:
  static const std::size_t MIN_HEADER_SIZE = sizeof(std::uint32_t) * 3;
 
  // Create empty (invalid) packet
  Packet();

  // Create packet from given data
  // NOTE: ownership over |data| is still on caller side.
  //       it will not be deallocated on packet destruction
  Packet(std::uint8_t* data, std::size_t size);
  Packet(const Packet&);
  Packet(Packet&&);
  Packet& operator=(const Packet&);
  Packet& operator=(Packet&&);
  ~Packet() = default;

  // check if packet is valid
  bool valid() const;

  // returns pointer to packet data (header included)
  const std::uint8_t* data() const;

  // returns packet total size (including header)
  std::size_t size() const;

  //  version (V): 2 bits
  //     This field identifies the version of RTP.  The version defined by
  //     this library is two (2).  (The value 1 is used by the first
  //     draft version of RTP and the value 0 is used by the protocol
  //     initially implemented in the "vat" audio tool.)
  std::uint8_t version() const;
  void set_version(std::uint8_t version);

  // padding (P): 1 bit
  // If the padding bit is set, the packet contains one or more
  // additional padding octets at the end which are not part of the
  // payload.  The last octet of the padding contains a count of how
  // many padding octets should be ignored, including itself.  Padding
  // may be needed by some encryption algorithms with fixed block sizes
  // or for carrying several RTP packets in a lower-layer protocol data
  // unit.
  bool has_padding() const;
  void set_has_padding(bool pbit);

  // extension (X): 1 bit
  // If the extension bit is set, the fixed header will be followed by
  // exactly one header extension
  bool has_extensions() const;
  void set_has_extensions(bool xbit);

  // CSRC count (CC): 4 bits
  // The CSRC count contains the number of CSRC identifiers that follow
  // the fixed header.
  std::uint8_t contributors_count() const;
  void set_contributors_count(std::uint8_t cc);

  // marker (M): 1 bit
  // The interpretation of the marker is defined by a profile.  It is
  // intended to allow significant events such as frame boundaries to
  // be marked in the packet stream.  A profile MAY define additional
  // marker bits or specify that there is no marker bit by changing the
  // number of bits in the payload type field.
  bool marked() const;
  void set_marked(bool mbit);

  // payload type (PT): 7 bits
  // This field identifies the format of the RTP payload and determines
  // its interpretation by the application.
  std::uint8_t payload_type() const;
  void set_payload_type(std::uint8_t type);

  // sequence number: 16 bits
  // The sequence number increments by one for each RTP data packet
  // sent, and may be used by the receiver to detect packet loss and to
  // restore packet sequence.
  std::uint16_t sequence() const;
  void set_sequence(std::uint16_t seq);

  // timestamp: 32 bits
  // The timestamp reflects the sampling instant of the first octet in
  // the RTP data packet.  The sampling instant MUST be derived from a
  // clock that increments monotonically and linearly in time to allow
  // synchronization and jitter calculations (see Section 6.4.1).  The
  // resolution of the clock MUST be sufficient for the desired
  // synchronization accuracy and for measuring packet arrival jitter
  // (one tick per video frame is typically not sufficient).  The clock
  // frequency is dependent on the format of data carried as payload
  // and is specified statically in the profile or payload format
  // specification that defines the format, or MAY be specified
  // dynamically for payload formats defined through non-RTP means.
  std::uint32_t timestamp() const;
  void set_timestamp(std::uint32_t timestamp);

  // SSRC: 32 bits
  // The SSRC field identifies the synchronization source.  This
  // identifier SHOULD be chosen randomly, with the intent that no two
  // synchronization sources within the same RTP session will have the
  // same SSRC identifier.
  std::uint32_t stream_id() const;
  void set_stream_id(std::uint32_t stream_id);

  // returns pointer to array of CSRC, 0 to 15 items
  // depending on value of contributors_count()
  std::uint32_t* contributors();

  // TODO
  // ExtensionsIterator extensions();

  std::size_t payload_size() const;

  // pointer to start of payload 
  std::uint8_t* payload();

private:
  // pointer to buffer containing the packet
  std::uint8_t* m_buffer;
  std::size_t   m_size;
};
}