...
namespace {
  ...
  shar::Packet encode_nal(const x265_nal& nal, const uint8_t temporal_id) {
...
namespace shar {
  ...
  std::vector<Packet> Encoder::encode(const Image& input) {
    ...
    for (auto i = 0; i < pi_nal; i++) {
      Packet curr_packet { encode_nal(out[i], 1) };
