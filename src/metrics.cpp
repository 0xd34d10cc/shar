...
MetricId Metrics::add(std::string name, Format format) {
...
void Metrics::remove(MetricId id) {
  assert(m_metrics.size() > id);
...
void Metrics::increase(shar::MetricId id, std::size_t delta) {
  assert(m_metrics.size() > id);
  if (m_metrics.size() > id) {
    m_metrics[id]->m_value.fetch_add(delta, std::memory_order::memory_order_relaxed);
  }
}
...
void Metrics::decrease(shar::MetricId id, std::size_t delta) {
  assert(m_metrics.size() > id);
  if (m_metrics.size() > id) {
    m_metrics[id]->m_value.fetch_sub(delta, std::memory_order_relaxed);
  }
}
