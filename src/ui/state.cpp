...
static void clipboard_copy(nk_handle usr, const char *text, int len) {
  ...
  str = static_cast<char *>(std::malloc(static_cast<usize>(len) + 1));

  if (!str)
    return;

  std::memcpy(str, text, static_cast<usize>(len));
  str[len] = '\0';
  SDL_SetClipboardText(str);
  free(str);
}

State::State() : m_context(std::make_unique<nk_context>()) {
  nk_init_default(m_context.get(), nullptr);
  m_context->clip.copy = clipboard_copy;
  m_context->clip.paste = clipboard_paste;
  m_context->clip.userdata = nk_handle_ptr(nullptr);
}
  ...
  if (event->type == SDL_KEYUP || event->type == SDL_KEYDOWN) {
    ...
    if (sym == SDLK_RSHIFT || sym == SDLK_LSHIFT)
      ...
      if (state[SDL_SCANCODE_LCTRL])
        ...
        nk_input_key(m_context.get(), NK_KEY_LEFT, down);
    } else if (sym == SDLK_RIGHT) {
      if (state[SDL_SCANCODE_LCTRL])
        nk_input_key(m_context.get(), NK_KEY_TEXT_WORD_RIGHT, down);
      else
        nk_input_key(m_context.get(), NK_KEY_RIGHT, down);
    } else
      return false;
    ...
    if (event->button.button == SDL_BUTTON_LEFT) {
      if (event->button.clicks > 1)
        nk_input_button(m_context.get(),
                        NK_BUTTON_LEFT /*NK_BUTTON_DOUBLE*/,
                        x,
                        y,
                        down);
    ...
    if (m_context.get()->input.mouse.grabbed) {
      int x = static_cast<int>(m_context.get()->input.mouse.prev.x);
      int y = static_cast<int>(m_context.get()->input.mouse.prev.y);
      nk_input_motion(m_context.get(),
                      x + event->motion.xrel,
                      y + event->motion.yrel);
    } else
      nk_input_motion(m_context.get(), event->motion.x, event->motion.y);
