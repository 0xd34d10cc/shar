...
static void assert_fails(BytesRef response_text, rtsp::Error e) {
...
TEST(rtsp_response, simple_response) {
  BytesRef simple_response = "RTSP/1.0 200 OK\r\n"
                             "\r\n";
...
TEST(rtsp_response, single_header) {
  BytesRef single_header = "RTSP/1.0 200 OK\r\n"
                           "CSeq: 7\r\n"
                           "\r\n";
  std::array<rtsp::Header, 16> headers;
  rtsp::Response response(rtsp::Headers{headers.data(), headers.size()});
...
TEST(rtsp_response, ends_wo_line_end) {
  std::array<rtsp::Header, 16> headers;
  rtsp::Response response(rtsp::Headers{headers.data(), headers.size()});
  auto size = response.parse("RTSP/1.0 200 OK\r\n"
                             "CSeq: 7");
...
TEST(rtsp_response, response_with_body) {
  BytesRef base_response = "RTSP/1.0 200 OK\r\n"
                           "CSeq: 2\r\n"
  ...
  EXPECT_EQ(headers[1],
            rtsp::Header("Content-Base", "rtsp://example.com/media.mp4"));
...
TEST(rtsp_response, too_many_headers) {
  BytesRef response_too_many_headers =
      "RTSP/1.0 200 OK\r\n"
      "CSeq: 2\r\n"
      "Content-Base: rtsp://example.com/media.mp4\r\n"
      "Content-Type: application/sdp\r\n"
